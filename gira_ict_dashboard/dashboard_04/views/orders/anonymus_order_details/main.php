<?php
    $order_id = ''; 
    if(isset($_GET['anonymus_order'])) {
        $order_id = $_GET['anonymus_order'];
    }
?>
...
<a href="./orders.php" class="text-blue-500 hover:underline border px-2 text-dark">&larr; Back to Orders</a>
...
<?php
    $select_order_details = mysqli_query(
        ...
        WHERE signed_out_order_id = '$order_id'"
    );

    foreach($select_order_details as $order) {
        ?>
           <div class="bg-white shadow-md  mt-4 p-6 flex flex-col md:flex-row">

                <!-- Product Image -->
                <div class="md:w-1/3">
                    <img src="./data/images/<?=$order['image_url']?>" alt="Product Image" class="w-full h-auto object-cover">
                    <?php
                ...
                <div class="md:w-2/3 md:pl-8 mt-4 md:mt-0">
                    ...
                    <div class="mt-4">
                        ...
                        <br>
                        <span class="border-2 px-2 my-3 font-bold">Quantity: <?=$order['quantity']?></span>
                    </div>
                    ...
                    <div class="flex mt-6 space-x-4">
                        ...
                        <?php
                            if($order['payment_status'] == 'pending') {
                                ?>
                                    <button onclick="window.location.replace('./services/payment/anonymus_pod.php?o= <?=$order['signed_out_order_id']?>&p= <?php echo'PDO'; ?>&a= <?=$order['price']?>&s= <?=$order['payment_status']?>&previous_page= <?=urldecode($_SERVER['REQUEST_URI'])?>')"  class="border font-semibold py-2 px-4 rounded-0 transition">
                                        Approve
                                    </button> 
                                <?php
                            } else {
                                ?>
                                    <button onclick="window.location.replace('./services/payment/undo_pod_payment.php?o= <?=$order['signed_out_order_id']?>&p= <?php echo'PDO'; ?>&a= <?=$order['price']?>&s= <?=$order['payment_status']?>&previous_page= <?=urldecode($_SERVER['REQUEST_URI'])?>')" class="alert-success border font-semibold py-2 px-4 rounded-0 transition">
                                        Delivered
                                    </button> 
                                <?php
                            }
