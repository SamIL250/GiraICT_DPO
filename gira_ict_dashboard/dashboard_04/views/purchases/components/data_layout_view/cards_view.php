...
<div class="" id="list_view">
                ...
                <div class="card my-3">
                    <div class="relative grid grid-cols-12 w-100 p-2 items-center justify-between">
                            ...
                            <div class="grid grid-cols-12 gap-3">
                                ...
                                <div class="col-span-9">
                                    <div class="grid grid-cols-1 items-center">
                                        <p class="font-bold cursor-pointer" onclick="window.location.replace('purchase_anon_details.php?anon_purchase_id=<?=$products['signed_out_order_id']?>')"><?=$products['product_name']?></p>
                        ...
                        <div class="col-span-3 items-center"> 
                            <a class=" rounded-0 px-3 py-1 bg-warning text-white" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#purchaseAnonymusDeleteModal" onclick="document.getElementById('puchaseAnonOrderId').value='<?=$products['signed_out_order_id'] ?>'"><i class="bi bi-trash"></i> Remove</a> 
                            <span class=" rounded-0 px-3 py-1 alert-success cursor-pointer" onclick="window.location.replace('./services/payment/undo_anonymus_pod_payment.php?o= <?=$products['signed_out_order_id']?>&p= <?php echo'PDO'; ?>&a= <?=$products['price']?>&s= <?=$products['payment_status']?>&previous_page= <?=urldecode($_SERVER['REQUEST_URI'])?>')">Derived</span>
                        </div>
                        <div class="col-span-2 items-center dropdown ">
                            <span class="border px-3 py-1" data-bs-toggle="dropdown" onclick="">More..</span> 
                        </div>
                        <?php
    ...
    <?php 
                            ...
                            <div class="grid grid-cols-12 w-100 p-2 items-center relative">
                                <div class="col-span-7 ">
                                    <div class="grid grid-cols-12 gap-3">
                                        ...
                                        <div class="col-span-9">
                                            <div class="grid grid-cols-1 items-center">
                                            <p class="font-bold cursor-pointer" onclick="window.location.replace('purchase_details.php?order_id=<?=$products['order_id']?>')"><?=$products['product_name']?></p>
                                                <div class="grid grid-cols-3 items-center gap-1"> 
                                                    <p><?=number_format($products['price']* $products['quantity'], 2). 'Frw'?></p>
                                                    <span class="border rounded-2xl text-center p-1"><?=$products['stock_name']?></span>
                                                </div>
                                ...
                                <div class="col-span-3 items-center"> 
                                    <a class=" rounded-0 px-3 py-1 bg-warning text-white" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="document.getElementById('productId').value='<?=$products['order_id']?>'"><i class="bi bi-trash"></i> Remove</a>
        ...
        } elseif(isset($_POST['product_search_name'])) {
            $search_name = $_POST['product_search_name'];
            $select_products = mysqli_query(
                $conn, 
                "SELECT 
                order_items.*,
                ...
                INNER JOIN stock ON products.stock_id = stock.stock_id
                WHERE orders.canceled = 'false'
                AND products.product_name LIKE'%$search_name%'
                AND orders.status = 'delivered'
                ORDER BY stock.stock_name ASC
                "
            );
